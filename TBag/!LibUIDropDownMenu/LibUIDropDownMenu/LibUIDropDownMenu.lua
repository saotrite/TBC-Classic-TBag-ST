...
-- ----------------------------------------------------------------------------
local MAJOR_VERSION = "TBag-LibUIDropDownMenu-4.0"
...
local function create_MenuButton(name, parent)
	...
	local fcw
	if WoWClassic then
		fcw = CreateFrame("Button", name.."ColorSwatch", f, nil)
		fcw:SetSize(16, 16)
...
function lib:UIDropDownMenu_AddButton(info, level)
	...
	if (not listFrame) then
		-- Ensure list frame exists; older versions could end up with index 0 and crash.
		if (self.UIDropDownMenu_CreateFrames) then
			self:UIDropDownMenu_CreateFrames(level, 1);
		end
		listFrame = _G["L_DropDownList"..level];
	end
	index = (listFrame and listFrame.numButtons and (listFrame.numButtons + 1)) or 1
	...
	if (not button) then
		-- Create frames/buttons as needed
		if (self.UIDropDownMenu_CreateFrames) then
			self:UIDropDownMenu_CreateFrames(level, index);
		end
		button = _G[listFrameName.."Button"..index]
		if (not button) then
			return;
		end
	end
	...
	if ( info.hasColorSwatch ) then
		if WoWClassic then
			-- local base = "L_DropDownList"..level.."Button"..index
			-- print("Checking:", base,
			-- _G[base.."ColorSwatchNormalTexture"],
			-- _G[base.."ColorSwatchNormalTexture2"],
			-- _G[base.."ColorSwatchNormalTexture3"],
			-- _G[base.."ColorSwatch".."NormalTexture"])
			-- _G["L_DropDownList"..level.."Button"..index.."ColorSwatch".."NormalTexture"]:SetVertexColor(info.r, info.g, info.b);
			local swatch = _G["L_DropDownList"..level.."Button"..index.."ColorSwatch"]
			if swatch and swatch.GetNormalTexture and swatch:GetNormalTexture() then
				swatch:GetNormalTexture():SetVertexColor(info.r, info.g, info.b)
			end
		else
