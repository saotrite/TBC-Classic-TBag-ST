...
function Inv.RightClickMenu_populate(self, level)
  ...
  elseif (TInvFrame.RightClickMenu_mode == "mainwindow") then
    if (level == 1) then
      ...
      info = { ["text"] = string.format(L["TBag%s"],TBag.VERSION), ["notClickable"] = 1, ["isTitle"] = 1, ["notCheckable"] = nil };
      TBag.LibDD:UIDropDownMenu_AddButton(info, level);
  ...
  if TBag and type(TBag.FixMenuFrameLevels)=="function" then
  TBag:FixMenuFrameLevels()
elseif UIDropDownMenu_FixMenuFrameLevels then
  UIDropDownMenu_FixMenuFrameLevels(TBag)
end
end
...
function Inv.UserDropdown_OnLoad(self)
  ...
  -- L_UIDropDownMenu_SetWidth actually adds 50 to our width, we really only want
