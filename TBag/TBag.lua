...
-- -----------------------------------------------------------------------
-- Compatibility shims for newer Classic clients (Anniversary/TBC, etc.)
-- -----------------------------------------------------------------------

-- LoadAddOn was deprecated/removed; use C_AddOns.LoadAddOn when available.
local function TBag_LoadAddOn(name)
  if type(_G.LoadAddOn) == 'function' then
    return _G.LoadAddOn(name)
  end
  if _G.C_AddOns and type(_G.C_AddOns.LoadAddOn) == 'function' then
    return _G.C_AddOns.LoadAddOn(name)
  end
  return nil, 'MISSING_API'
end

-- Container APIs were moved under C_Container on newer clients.
if not _G.GetContainerNumSlots and _G.C_Container and type(_G.C_Container.GetContainerNumSlots) == 'function' then
  function _G.GetContainerNumSlots(bag) return _G.C_Container.GetContainerNumSlots(bag) end
end

if not _G.GetContainerItemLink and _G.C_Container and type(_G.C_Container.GetContainerItemLink) == 'function' then
  function _G.GetContainerItemLink(bag, slot) return _G.C_Container.GetContainerItemLink(bag, slot) end
end

-- GetContainerNumFreeSlots moved under C_Container on newer clients.
if not _G.GetContainerNumFreeSlots and _G.C_Container and type(_G.C_Container.GetContainerNumFreeSlots) == 'function' then
  function _G.GetContainerNumFreeSlots(bag) return _G.C_Container.GetContainerNumFreeSlots(bag) end

-- ContainerIDToInventoryID moved under C_Container on newer clients.
if not _G.ContainerIDToInventoryID and _G.C_Container and type(_G.C_Container.ContainerIDToInventoryID) == 'function' then
  function _G.ContainerIDToInventoryID(containerID) return _G.C_Container.ContainerIDToInventoryID(containerID) end
end

end

-- Old API returned multiple values; C_Container returns a table.
if not _G.GetContainerItemInfo and _G.C_Container and type(_G.C_Container.GetContainerItemInfo) == 'function' then
  function _G.GetContainerItemInfo(bag, slot)
...
if not _G.SplitContainerItem and _G.C_Container and type(_G.C_Container.SplitContainerItem) == 'function' then
  function _G.SplitContainerItem(bag, slot, amount) return _G.C_Container.SplitContainerItem(bag, slot, amount) end
end

if not _G.PickupContainerItem and _G.C_Container and type(_G.C_Container.PickupContainerItem) == 'function' then
  function _G.PickupContainerItem(bag, slot) return _G.C_Container.PickupContainerItem(bag, slot) end
end

if not _G.PutItemInBackpack and _G.C_Container and type(_G.C_Container.PutItemInBackpack) == 'function' then
  function _G.PutItemInBackpack() return _G.C_Container.PutItemInBackpack() end
end

if not _G.PutItemInBag and _G.C_Container and type(_G.C_Container.PutItemInBag) == 'function' then
  function _G.PutItemInBag(containerID) return _G.C_Container.PutItemInBag(containerID) end
end

-- Helper to hook either global functions (older clients) or namespaced ones (newer clients).
local function TBag_HookContainerFunc(funcName, handler)
  if type(_G[funcName]) == 'function' then
    hooksecurefunc(funcName, handler)
    return true
  end
  if _G.C_Container and type(_G.C_Container[funcName]) == 'function' then
    hooksecurefunc(_G.C_Container, funcName, handler)
    return true
  end
  return false
end
...
--    TMailItm[self.S_VERSION] = 1;
  ...
  -- Keybinding UI differs on newer Classic clients; avoid loading Blizzard_BindingUI when it is not compatible.
  -- Only load it if the old OptionsList-based UI is present.
  if type(_G.OptionsList_ClearSelection) == "function" and _G.OptionsFrameListTemplate then
    TBag_LoadAddOn("Blizzard_BindingUI");
  end
end
...
function TBag:MakeColorMenu(cfg, updatefunc, level, bagarr)
  ...
  info = { ["disabled"] = 1, ["text"] = "", ["notCheckable"] = true };
  TBag.LibDD:UIDropDownMenu_AddButton(info, level);
...
--                if (itemitm[self.I_TYPE] ~= L["Container"]) then
                if (itemitm[self.I_TYPE] ~= "1" and itemitm[self.I_TYPE] ~= "11") then
                ...
                if (bagtype and itmfam) then
                  if (bagtype == 1 and itmfam == 1) or
                    ...
                    (bagtype == 7 and itmfam == 7) or
                    (bagtype == 256 and itmfam == 9) then
...
TBag_HookContainerFunc('SplitContainerItem', TBag.SplitContainerItem);
...
TBag_HookContainerFunc('PickupContainerItem', TBag.PickupContainerItem);
...
-- The L_UIDropDownMenu code depends on it working properly.  When it doesn't work
...
function TBag:FixMenuFrameLevels()
  for l=1,L_UIDROPDOWNMENU_MAXLEVELS do
    for b=1,L_UIDROPDOWNMENU_MAXBUTTONS do
